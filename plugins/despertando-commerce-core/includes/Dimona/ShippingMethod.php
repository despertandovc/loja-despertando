...
final class ShippingMethod
{
    ...
    {
        add_action('woocommerce_shipping_init', [self::class, 'defineMethod']);
        add_filter('woocommerce_shipping_methods', [self::class, 'registerMethod']);
    }

    public static function defineMethod(): void
    {
        if (class_exists(__NAMESPACE__ . '\\WooCommerceShippingMethod')) {
            return;
        }

        class WooCommerceShippingMethod extends \WC_Shipping_Method
        {
